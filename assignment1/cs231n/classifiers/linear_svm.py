...
def svm_loss_naive(W, X, y, reg):
    ...
    """
    dW = np.zeros(W.shape) # initialize the gradient as zero 初始化
    ...
    for i in range(num_train):
        ...
        # print(X[i].shape, W.shape) # (3073,) (3073, 10)
...
def svm_loss_vectorized(W, X, y, reg):
    ...
    num_train = X.shape[0]
    scores = X.dot(W)
    scores -= scores[range(num_train), [y]].T # (500,10) - (500,1) 但是不转置的话是(1,500)
    scores += 1
